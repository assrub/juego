...
class Bala(pygame.sprite.Sprite): 
    def __init__(self, x, y, velocidad, posicion):
        super().__init__()
        self.x = x
        self.y = y
    ...
    def mover(self):
        if self.posicion == 'izq' and self.x > 0: 
            self.x -= self.velocidad
        elif self.posicion == 'der' and self.x < TAM_PANTALLA_X: 
            self.x += self.velocidad
        elif self.posicion == 'arr' and self.y > 0: 
            self.y -= self.velocidad
        elif self.posicion == 'abj' and self.y < TAM_PANTALLA_Y: 
            self.y += self.velocidad
